    ...
    <div className="min-h-screen bg-[#0a0a0a] flex">
      ...
      <main className="flex-1 lg:ml-64 min-w-0">
        <div className="sticky top-0 z-20 bg-[#0a0a0a]/90 backdrop-blur-md border-b border-white/5 px-6 h-16 flex items-center mt-14 lg:mt-0">
          ...
          <span className="ml-3 text-xs text-[#888888] bg-white/5 px-2 py-1 rounded-full">{MEMBERS.length} members</span>
        </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Search by name or email..." className="w-full bg-[#111111] border border-white/5 rounded-xl pl-9 pr-4 py-2.5 text-white text-sm focus:outline-none focus:border-white/20 transition-colors" />
            </div>
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
              className="bg-[#111111] border border-white/5 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none">
                    ...
                    <tr key={m.id} className="hover:bg-white/2 transition-colors cursor-pointer">
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 rounded-full bg-[#DC2626]/10 flex items-center justify-center shrink-0">
                            <span className="text-[#DC2626] text-xs font-bold">{m.name[0]}</span>
                          </div>
                      ...
                      <td className="px-4 py-3.5 text-[#888888] text-xs">{m.joined}</td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-4 py-3.5">
                        <span className={`text-xs font-medium px-2 py-1 rounded-full ${m.status === 'Active' ? 'bg-green-500/10 text-green-400' : 'bg-white/5 text-[#888888]'}`}>
                          {m.status}
                        </span>
                      </td>
                    </tr>
              ...
              {filtered.map(m => (
                <div key={m.id} className="px-4 py-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${m.status === 'Active' ? 'bg-green-500/10 text-green-400' : 'bg-white/5 text-[#888888]'}`}>{m.status}</span>
                  </div>
                  <p className="text-[#888888] text-xs mt-0.5">{m.email} · {m.mrr}/mo · {m.gyms} gym{m.gyms > 1 ? 's' : ''}</p>
                </div>
              ))}
            </div>
          </div>

    ...
    <div className="min-h-screen bg-[#0D0D0D] p-6">
      ...
      <div className="bg-[#111] border border-white/5 rounded-xl overflow-hidden">
        ...
        <div className="flex gap-4 px-5 py-3 border-b border-white/5">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-3 w-24 bg-[#222222] rounded animate-pulse" />
          ))}
        </div>
        {/* Table rows */}
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="flex gap-4 items-center px-5 py-4 border-b border-white/5 last:border-0">
            <div className="h-8 w-8 bg-[#222222] rounded-full animate-pulse shrink-0" />
            <div className="h-4 w-32 bg-[#222222] rounded animate-pulse" />
            <div className="h-4 w-20 bg-[#222222] rounded animate-pulse ml-auto" />

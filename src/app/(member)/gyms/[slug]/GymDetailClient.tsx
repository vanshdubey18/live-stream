...
const DISCIPLINE_COLORS: Record<string, string> = {
  ...
  MMA: 'bg-[#DC2626]/10 text-[#DC2626] border-[#DC2626]/20',
...
const LEVEL_COLORS: Record<string, string> = {
  ...
  Advanced: 'text-[#DC2626] bg-[#DC2626]/10',
}
...
export default function GymDetailClient({ gym, coaches, sessions, memberCount, membership, isLoggedIn }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      {/* Back nav */}
      <div className="sticky top-0 z-20 bg-[#0a0a0a]/90 backdrop-blur-md border-b border-white/5 px-6 h-14 flex items-center gap-3">
        <a href="/gyms" className="flex items-center gap-1.5 text-[#888888] hover:text-white text-sm transition-colors">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="bg-[#111111] border border-white/5 rounded-2xl p-8">
          <div className="flex flex-col sm:flex-row items-start gap-6">
            <div className="w-20 h-20 rounded-2xl bg-[#DC2626]/15 flex items-center justify-center shrink-0">
              <span className="text-[#DC2626] font-black text-3xl">{gym.name[0]}</span>
            </div>
            <div className="flex-1 min-w-0">
              ...
              {gym.description && (
                <p className="text-[#888888] text-sm mt-3 leading-relaxed">{gym.description}</p>
              )}
            ...
            <div className="shrink-0 flex flex-col items-end gap-3">
              <div className="text-right">
                ...
                <p className="text-[#888888] text-xs flex items-center gap-1 justify-end"><Users size={11} /> members</p>
              </div>
              {joined ? (
                ...
                <button
                  ...
                  className="bg-[#DC2626] hover:bg-red-700 text-white font-bold px-6 py-2.5 rounded-xl text-sm transition-colors">
                  Join Gym
                </button>
              )}
            </div>
        ...
        {liveSessions.length > 0 && (
          <div className="bg-[#DC2626]/10 border border-[#DC2626]/30 rounded-2xl px-6 py-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <span className="w-2.5 h-2.5 rounded-full bg-[#DC2626] animate-pulse" />
              <div>
                <p className="text-[#DC2626] text-xs font-bold uppercase tracking-wide">Live Now</p>
                <p className="text-white font-semibold text-sm">{liveSessions[0].title}</p>
              </div>
            </div>
            {joined ? (
              <a href={`/watch/${liveSessions[0].id}`}
                className="bg-[#DC2626] hover:bg-red-700 text-white font-bold px-5 py-2 rounded-xl text-sm transition-colors flex items-center gap-1.5">
                <Radio size={14} /> Watch
              </a>
            ) : (
              <span className="text-[#888888] text-xs">Join to watch</span>
            )}
          </div>
        )}
        ...
        {coaches.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {coaches.map((c: any) => (
                <div key={c.id} className="bg-[#111111] border border-white/5 rounded-2xl p-5 flex items-start gap-4">
        ...
        <section>
          ...
          {upcomingSessions.length === 0 ? (
            <div className="bg-[#111111] border border-white/5 rounded-2xl px-6 py-10 text-center">
              <p className="text-[#888888] text-sm">No upcoming classes scheduled yet.</p>
            </div>
          ) : (
              ...
              <div className="divide-y divide-white/5">
                {upcomingSessions.map((s: any) => (
                  <div key={s.id} className="flex items-center gap-4 px-5 py-4">
                    <div className={`flex items-center gap-1.5 shrink-0 w-5 ${s.status === 'live' ? 'text-[#DC2626]' : 'text-[#555]'}`}>
                      {s.status === 'live' && <span className="w-1.5 h-1.5 rounded-full bg-[#DC2626] animate-pulse" />}
                      <Calendar size={14} />
                    </div>
                    <div className="flex-1 min-w-0">
        ...
        {/* CTA if not joined */}
        {!joined && (
          <div className="bg-[#111111] border border-white/5 rounded-2xl px-8 py-10 text-center space-y-4">
            <h3 className="text-white font-black text-xl">Ready to train at {gym.name}?</h3>
            <p className="text-[#888888] text-sm">Join to watch live classes and replays from anywhere.</p>
            <button
              onClick={() => isLoggedIn ? setShowJoin(true) : window.location.href = `/signup?redirectTo=/gyms/${gym.slug}`}
              className="bg-[#DC2626] hover:bg-red-700 text-white font-bold px-8 py-3 rounded-xl text-sm transition-colors">
              Join {gym.name}
            </button>
          </div>
        )}
